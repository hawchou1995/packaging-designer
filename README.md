...
| `轴测图-*.png / .svg` | 独立轴测图 |
| `参数表.md` | 尺寸链、用纸面积、格数/层数/收容数等全部计算值 |
...
**片材**：长 / 宽 / 厚（+ 名称、材料）。
输出第一角三视图（俯视/主视/侧视 + 等轴测）+ 1:1 轮廓 DXF + STEP/STL + 参数表；A3 比例自动选档。
...
- 楞型来自**公司包材汇总表**：A版单瓦（B 楞）、A版双瓦（BC 楞）、超密封 K 单瓦（C 楞）、超密封 K 双瓦（高强 BC）、ABC 三层、AAA 三层（厚度 / 边压 / 耐破齐全）。
...
- 比例按 GB/T 14690 数系自动选档（1:1.5 / 2 / 2.5 / 3 / 4 / 5 / 6 …），尺寸线避让、尺寸文字白底遮罩。
- 材料口径参考 GB/T 6543-2025、GB/T 6544；箱型按 FEFCO 官方目录口径。

## 安装与运行

下载 `包装设计器-1.0.0-setup.exe`（amd64，约 108 MB），双击安装。**免管理员**：默认装到 `%LOCALAPPDATA%\Programs\PackagingDesigner`，自带 Python 运行时与全部依赖，目标机无需装任何东西。
