...
**v1.0.15 修复（0310 围框+两盖：楞型可随意搭配 / 新增固定盖高）**

- **围框与两盖可随意搭配楞型**（现场：围框双瓦 + 盖单瓦，一点算就报 `glue_w = 35 超出标准区间 45–50`）。根因：**接舌只在围框上**（两盖是无接舌的角片盘），可接舌宽的默认值取自「上盖」楞型、校验却拿围框楞型去比 —— 混搭时一个接舌宽不可能同时落进两档区间。现改为接舌宽的**默认值 / 标准区间 / 校验一律按围框楞型**，界面上该行标注「按围框楞型」。
- **顺带修掉第二处混搭阻塞**：`check()` 里那条尺寸链等式隐含「上盖必须是最厚的板」，上下盖板厚不同时必然失败。现改为物理正确形式 —— **围框外 + 2×间隙 + 2×较厚盖厚 = 盖外**，并逐只盖检查「围框套得进盖内腔」；参数表注明围框按较厚盖配间隙、薄盖单边多出的余量。
- **新增「盖高口径」（默认固定高度 100）**：原来两盖高度固定为「围框高/2」（腰线对接）；现场实际用**固定盖高 100**（中段围框外露）。现在可选 **固定高度 / 整体一半**，默认固定高度、盖高可调（默认 100），需要旧口径一键切回。两盖高度之和超过围框高时给出可行动的报错（`盖高改 ≤ 93，或整体外高 ≥ 214，或改选「整体一半」`），**不静默改你填的值**。
- **两处小修**：盒型页的「标准区间」提示行此前没显示（建行时没给提示位）—— 现在正常显示；提示来源串改按楞型档（原来一律显示 0310 的间隙说明）。
- **版面缺陷（现场 2026-09-23 指出：图纸压到说明文字 / 图框）**：自动比例把 60 / 80mm 这类**纸面预留**加进「数据尺寸」里再一起除以比例 —— 1:8 时那 80mm 只剩 10mm，视图实际比预算多下探几十毫米，压进说明块与标题栏。现在**数据尺寸归数据、纸面预留从可用区里扣**；并把这些散落的同类问题一并修掉：① 说明块**一律折行到标题栏左侧**（38 + 185 < 228）；② 两盖间距不够时省掉会被相邻尺寸列压住的内部标签；③ 网格 C 视图三条尺寸行改**纸面行距**（原来用数据 16mm 行距，1:15 时三行只差 1mm，标签自己叠自己）；④ 网格视图名居中钳进内框；⑤ 0312 底箱视图名钳位；⑥ 仿形块视图名让开总长尺寸标签。
- **门禁补强（这轮的教训：检测器看不见 = 等于没检）**：`layout_check.py` 新增「版面体检」——把全部内容统一折算成纸面 mm，判 **越框 / 压标题栏 / 压字 / 线压说明**，任一非零即 `exit 1`；并补进 5 个「曾经真出问题」的算例。旧检查器看不见这些：A) 只在**同一坐标轴内**比文字与线，且带白底遮罩的文字直接跳过；B) 只看文字**位置**是否越出内框。已用变异测试验证：把比例预算改回旧写法 → 门禁报 9 处问题并 exit 1。

验证：新增 `verify_v115.py` 25/25（混搭楞型 / 上下盖板厚不同 / 盖高超限 / 边界 H=2×盖高+t上+t下 / 内尺寸口径 / 界面默认值与显隐）；`check_gui.py` 的 0310 用例改为「围框 BC + 上盖 B + 下盖 C」混搭 + 盖高两口径；`reg_check` 几何回归 PASS。
...
下载 `包装设计器-1.0.15-setup.exe`（amd64，约 108 MB），双击安装。**免管理员**：默认装到 `%LOCALAPPDATA%\Programs\PackagingDesigner`，自带 Python 运行时与全部依赖，目标机无需装任何东西。
...
python layout_check.py     :: 版面体检：越框 / 压标题栏 / 压字 / 线压说明（有问题 exit 1）
python verify_v115.py      :: 0310 混搭楞型接舌宽 + 盖高两口径回归
